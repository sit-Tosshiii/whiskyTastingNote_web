...
- JSON エクスポート / インポート（`/tools/data` ページ）
...
IndexedDB 内のデータはブラウザ開発者ツール、もしくは `http://localhost:3000/tools/data` ページの「データ入出力ツール」から JSON でエクスポート / インポートできます。同ページには全ノート削除ボタンもあるため、実行前に必ずバックアップを取得してください。操作手順は `docs/data_seeding.md` を参照してください。
